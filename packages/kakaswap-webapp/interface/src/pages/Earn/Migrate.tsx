...
import { ChainId, Token, WAVAX } from '@jean1011/kakarot'
...
export default function Migrate({
  ...
  const canZap = (pglFrom?.involvesToken(PNG[ChainId.WAGMI]) && pglTo?.involvesToken(PNG[ChainId.WAGMI]))
    || (pglFrom?.involvesToken(WAVAX[ChainId.WAGMI]) && pglTo?.involvesToken(WAVAX[ChainId.WAGMI]))
  ...
  const addTokenButton = (token: Token | undefined) => {
    if (!token) return
    if (token.equals(PNG[ChainId.WAGMI])) return
    if (token.equals(WAVAX[ChainId.WAGMI])) return
    return (
      <ButtonPrimary
        width={'250'}
        onClick={() => {
